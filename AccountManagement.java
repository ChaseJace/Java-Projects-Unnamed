...
public void setInfo(String name, String gender, String age) throws IOException { //throws	any	error- you	can	use	try	and catch
this.name=name;
this.gender=gender;
this.age=age;
...
fWriter.close(); //storing	to	file	will	not	work	without	a	closing	statement
}
...
while(read.hasNext()) {
//reads	data	every	after	space	from	a	textfile,	stores it	to	a	certain	variable
name=read.next();
gender=read.next();
age=read.next();
...
//if	the	entered	username is	the	same	as the	username from	the	file	and	when	the	password	entered	is	also	the	same	as the	file	upon	decryption
if(username.equals(this.username) && password.equals(original)) {
hold="Login	successful.";
}else { hold="Login	failed."; }
					return hold;
} //end	of	method
}